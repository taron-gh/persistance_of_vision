...
void setup() {
  ...
  if (EEPROM.read(100) != 1) {   // в ячейке 100 должен быть замисан флажок 1, если его нет - делаем
    ...
    EEPROM.updateByte(105, 0);
         ...
         // пин кнопки подтянуть

            // если при запуске нажата кнопка
...
void showAnimation() {
  for (;;) {                        // бесконечный цикл отрисовки
    ...
    else{      // если цикл ВКЛЮЧЕН, но ОТПУЩЕНА кнопка, покинуть цикл отрисовки
